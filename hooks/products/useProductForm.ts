...
/**
 * Configuración para el hook useProductForm
 */
export interface UseProductFormConfig<T extends FormState = FormState> {
...
/**
 * Hook genérico para manejar acciones de productos (crear, actualizar, eliminar)
 * Maneja automáticamente los toasts cuando la acción termina de ejecutarse
 * Compatible con la estructura de FormData y FormState como useCustomerForm
 *
 * @example
 * const { formState, handleSubmit, isPending } = useProductForm({
 *   action: createProductAction,
 *   successMessage: '¡Producto creado exitosamente!',
 *   onSuccess: () => closeModal(),
 * });
 */
export function useProductForm<T extends FormState = FormState>({
  action,
